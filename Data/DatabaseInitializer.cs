        ...
        public static async void SeedData(DronesContext context)
        {
            if (!context.Drones.Any())
            {
                await context.Drones.AddRangeAsync(
                    ...
                    new Drone { Id = Guid.NewGuid(), SerialNumber = "DRON-02", Model = ModelEnum.Middleweight.ToString(), WeightLimit = 500, BatteryCapacity = 50, State = StateEnum.IDLE.ToString() },
                    ...
                    new Drone { Id = Guid.NewGuid(), SerialNumber = "DRON-05", Model = ModelEnum.Lightweight.ToString(), WeightLimit = 500, BatteryCapacity = 20, State = StateEnum.IDLE.ToString() }
            ...
            if (!context.Items.OfType<Medication>().Any())
            {
                await context.Items.AddRangeAsync(
                    ...
                    new Medication { Id = Guid.NewGuid(), Name = "B-COMPLEX", Weight = 200, Code = "MEDIC-04", Image = string.Empty },
                    new Medication { Id = Guid.NewGuid(), Name = "VITAMIN C", Weight = 400, Code = "MEDIC-05", Image = string.Empty }
